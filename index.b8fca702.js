...
function renderBoard() {
    ...
    state.forEach((row, r)=>{
        row.forEach((cellValue, c)=>{
            ...
            if (cellValue !== 0) cellEl.classList.add(`field-cell--${cellValue}`);
        });
    });
    // aktualizacja wyniku
...
class Game {
    ...
    hasMoves() {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 3; c++){
            if (this.state[r][c] === this.state[r][c + 1]) return true;
        }
        // czy sąsiednie komórki można scalić w pionie?
        for(let c = 0; c < 4; c++)for(let r = 0; r < 3; r++){
            if (this.state[r][c] === this.state[r + 1][c]) return true;
        }
